...
/*********************************************************************/	
	...
	while(1)
	{
		...
		tVideoZoom.iWidth = ptVideoBufOut.iWidth/2;
		tVideoZoom.iHeight = ptVideoBufOut.iHeight/2;
		...
		{
		   	tVideoZoom.aucPixelDatas = malloc(tVideoZoom.iTotalBytes);
		}
		PicZoom(&ptVideoBufOut,&tVideoZoom);
		/* 显示转化的数据:已经转化为16bpp了 */
		memcpy(fbmem,tVideoZoom.aucPixelDatas,tVideoZoom.iTotalBytes);
