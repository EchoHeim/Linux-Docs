...
static int major=0;
static int key_dev=100;			
static unsigned int key_val=0;		//定义返回键值；
static unsigned int key_ev;			//定义阻塞标志位；
...
void fun_timer(unsigned long data)
{
	key_ev=1;
	wake_up_interruptible(&key_wait);		//唤醒读操作的阻塞等待；
}
...
//	printk("%d  ",*(int *)dev);			//传递的参数；
	mod_timer(&timer,jiffies+msecs_to_jiffies(20));		//修改定时器延时; 
	return 0;
}
