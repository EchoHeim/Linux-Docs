...
extern const unsigned char fontdata_12[];
extern const unsigned char fontdata_16[];
extern const unsigned char fontdata_24[];
extern const unsigned char fontdata_32[];
extern const unsigned char fontdata_48[];
...
void show_point(unsigned int x, unsigned int y, unsigned int c)
{
    ...
    *(mydata.fbp + location + 0) = c >> 0  & 0xff;  /* 蓝色的色深 */  
...
{/*显示字符串*/
    ...
    while(*ptr != '\0') 
	{
        if (*ptr<128)  /*判断是否是字符(字符的ASCII范围为0-127)*/
		{
			show_char(x, y, forecolor, backcolor, *ptr, size, mode);
            x += size/2;
			if(x>799)
			{
				x = 0;
		...
		{    
        	show_chinese(x, y, forecolor, backcolor,ptr, size, mode);
            x += size;
			if(x>799)
			{
				x = 0;
...
void show_char(int x, int y,unsigned int forecolor,unsigned int backcolor,unsigned char ch,char size,char mode)
{
	...
	switch(size)
	{
		case 12:dots = (unsigned char *)&fontdata_12[(ch-32)*(size*size/12)];break;
    	case 16:dots = (unsigned char *)&fontdata_16[ch*(size*size/16)];break;
    	case 24:dots = (unsigned char *)&fontdata_24[(ch-32)*(size*size/12)];break;
    	case 32:dots = (unsigned char *)&fontdata_32[(ch-32)*(size*size/16)];break;
    	case 48:dots = (unsigned char *)&fontdata_48[(ch-32)*(size*size/16)];break;
    	case 64:dots = (unsigned char *)&fontdata_64[(ch-32)*(size*size/16)];break;
    }
    
    if(size%16 == 0)
    {
	    for (i = 0; i < (size*size/16); i++)
		{
			if(i%(size/16)==0)	y++;
	        for (j = 7; j >= 0; j--)
			{
	            if (dots[i] & (1 << j)) 
	                show_point(x + 7 - j + (i % (size/16)) * 8, y, forecolor);
	            else if(mode&0x01)
	                show_point(x + 7 - j + (i % (size/16)) * 8, y, backcolor);
	        }
	    }
    }else
    {
    	for (i = 0; i < (size*size/12); i++)
		{
			...
			{
	            if (dots[i] & (1 << j)) 
	                show_point(x + 7 - j + (i % (size/12)) * 8, y, forecolor);
	            else if(mode&0x01)
	                show_point(x + 7 - j + (i % (size/12)) * 8, y, backcolor);
	        }
	    }
    }
}
...
void Read_from_HZK(const unsigned char *s, char* const chs,char size)
{
    ...
    switch (size)
    {
    	...
    	case 24:if((fp = fopen("HZK24", "r")) == NULL) return;	break;//打开字库文件
    	case 32:if((fp = fopen("HZK32", "r")) == NULL) return;	break;//打开字库文件
    	case 40:if((fp = fopen("HZK40", "r")) == NULL) return;	break;//打开字库文件
        case 48:if((fp = fopen("HZK48", "r")) == NULL) return;	break;//打开字库文件
        case 64:if((fp = fopen("HZK64", "r")) == NULL) return;	break;//打开字库文件
    }
...
{           
    ...
    for(t=0;t<len;t++)
    {
        ...
        if(enshow==0&&t<(len-1))
        {
            ...
            {
            	if(mode&0x10)
               		show_char(x+(size/2)*t,y,point_color,back_color,'0',size,mode);
               	else
               		show_char(x+(size/2)*t,y,point_color,back_color,' ',size,mode);
                continue;
            }else enshow=1; 
        }
        show_char(x+(size/2)*t,y,point_color,back_color,temp+'0',size,mode); 
    }
}
