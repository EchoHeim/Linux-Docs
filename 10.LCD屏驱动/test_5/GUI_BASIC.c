...
{  u32  i;
   ...
   {  GUI_RLine(x0, y0, y1, linec);
	 ...
	 y1--;
   while(y0<=y1)						
   {  GUI_HLine(x0, y0, x1, fullc);	// 当前画水平线
      y0++;							// 下一行
   }
}
...
{  int  draw_x0, draw_y0;			// 刽图点坐标变量
   ...
   {  GUI_Point(draw_x0, draw_y0, fullc);	// 90度
   }
    	
   draw_x2 = draw_x3 = x0;
   draw_y2 = draw_y3 = y0 - r;
   if(draw_y2>=0)
   {  GUI_Point(draw_x2, draw_y2, fullc);	// 270度
   }
  	
   draw_x4 = draw_x6 = x0 + r;
   draw_y4 = draw_y6 = y0;
   if(draw_x4<GUI_LCM_XMAX) 
   {  GUI_Point(draw_x4, draw_y4, fullc);	// 0度
      fill_x1 = draw_x4;
   }
   else
   ...
   draw_y5 = draw_y7 = y0;
   if(draw_x5>=0) 
   {  GUI_Point(draw_x5, draw_y5, fullc);	// 180度
   }
   if(1==r) return;
...
{  int  draw_x0, draw_y0;			// 刽图点坐标变量
   ...
   int  center_x, center_y;		// 椭圆中心点坐标变量
   int  radius_x, radius_y;		// 椭圆的半径，x轴半径和y轴半径
